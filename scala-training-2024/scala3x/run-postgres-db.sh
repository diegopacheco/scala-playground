...
# execute the sql using src/main/sql/create.sql name the db vening
...
echo "Query the db... "
echo "SELECT * FROM PRODUCTS;" | PGPASSWORD=$POSTGRES_PASSWORD psql -U postgres -h 127.0.0.1 -p 5433 -d vending
echo "SELECT * FROM SALES;" | PGPASSWORD=$POSTGRES_PASSWORD psql -U postgres -h 127.0.0.1 -p 5433 -d vending
echo "SELECT * FROM TOLL;" | PGPASSWORD=$POSTGRES_PASSWORD psql -U postgres -h 127.0.0.1 -p 5433 -d vending
